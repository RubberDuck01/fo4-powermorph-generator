    ...
    internal class PowerMorphGenerator {
        ...
        private Dictionary<string, Dictionary<string, double>> UpdatePresetSliders(Dictionary<string, Dictionary<string, double>> presetsDict, Dictionary<string, double> defaultSliders) {
            ...
            foreach (var preset in presetsDict) {
                ...
                var sliders = preset.Value;
                var updatedSliders = new Dictionary<string, double>();

                foreach (var slider in sliders) {
                    string sliderName = slider.Key;
                    double sliderValue = slider.Value;

                    defaultSliders.TryGetValue(sliderName, out double baseValue);
                    double updatedValue;

                    if (sliderValue < 0) {
                        updatedValue = sliderValue + (Math.Abs(baseValue) / 100.0);
                    } else {
                        updatedValue = sliderValue - (baseValue / 100.0);
                    }

                    // Check if changed:
                    if (Math.Abs(updatedValue - sliderValue) > 0.0001) {
                        updatedSliders[sliderName] = updatedValue;
                    } else {
                        updatedSliders[sliderName] = sliderValue;
                    }
                }
                updatedPresets[presetName] = updatedSliders;
            }

            return updatedPresets;
        }

        private void WriteUpdatedTemplatesIni(string templatesIniPath, Dictionary<string, Dictionary<string, double>> updatedPresets) {
            using (var writer = new StreamWriter(templatesIniPath, false, Encoding.UTF8)) {
                foreach (var preset in updatedPresets) {
                    string presetName = preset.Key;
                    var sliders = preset.Value;

                    var sliderEntries = sliders.Select(slider => {
                        string sliderName = slider.Key;
                        double sliderValue = slider.Value / 100.0; // normalize
                        return $"{sliderName}@{sliderValue:0.####}";
                    });

                    string slidersLine = string.Join(", ", sliderEntries);
                    writer.WriteLine($"{presetName}={slidersLine}");
                }
            }
        }
        ...
        public void UpdateTemplatesIniForDefaultBody(BodyLoader bodyLoader, string sizeFilter) {
            try {
                ...
                var presetsDict = bodyLoader.ParseLoadedPresets(sizeFilter);
                var updatedPresets = UpdatePresetSliders(presetsDict, defaultSliders);

                string templatesIniPath = Path.Combine(_outputDirectory, "templates.ini");
                WriteUpdatedTemplatesIni(templatesIniPath, updatedPresets);
